...
#include <ultrassom/ultrassom.h>              // Biblioteca personalizada para controle do sensor ultrassônico
...
uint16_t duty_55  = TPM_MODULE * 0.55;       // 55% de duty cycle
...
uint16_t duty_35  = TPM_MODULE * 0.35;       // 35% de duty cycle (meio brilho)
uint16_t duty_40  = TPM_MODULE * 0.4;       // 40% de duty cycle (meio brilho)
uint16_t duty_60  = TPM_MODULE * 0.6;       // 50% de duty cycle (meio brilho)
uint16_t duty_65  = TPM_MODULE * 0.65;       // 50% de duty cycle (meio brilho)
...
{  
    ...
    pwm_tpm_Init(TPM2, TPM_PLLFLL, TPM_MODULE, TPM_CLK, PS_16, EDGE_PWM);
   ...
   //Definição dos estados iniciais dos pinos
    ...
    for (;;){  
        ...
        double d = ultrassom_get_distancia();

        if (d > 28.5 || d < 15.0){ // Linha reta            
            ...
            gpio_pin_set(gpio_b, 1, 1);
            gpio_pin_set(gpio_a, 4, 1);
            gpio_pin_set(gpio_a, 5, 0);
            pwm_tpm_CnV(TPM2, 0, duty_100);
            pwm_tpm_CnV(TPM2, 1, duty_55);
